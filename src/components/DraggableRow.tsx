    ...
    if (isEditing) {
        ...
        return (
            <div ref={setNodeRef} style={style}
                className="flex items-center gap-3 px-3 py-2.5 border-b border-foreground/10 last:border-b-0 bg-foreground/[0.03] overflow-hidden max-w-full">
                <div className={`flex-1 min-w-0 grid grid-cols-1 gap-2 ${groups.length > 0 ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
                    ...
                    {groups.length > 0 && (
                        <select
                            ...
                            disabled={isEditLoading}
                            className="h-8 rounded-md border border-foreground/15 bg-foreground/[0.05] px-2 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                        >
